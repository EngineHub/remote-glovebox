...
use actix::{Actor, AtomicResponse, Context as ActixContext, Handler, Message};
...
use crate::maven::maven_repo::{MavenCoords, MavenRepo, MavenUrlCoords};
use crate::maven::transport::http::HttpTransport;
...
pub enum Error {
    ...
    #[error(transparent)]
    Maven(#[from] crate::maven::maven_repo::Error),
    #[error(transparent)]
...
impl JarManager {
    pub fn new(
        ...
        Ok(JarManager {
            maven: MavenRepo::new(HttpTransport::new(maven)),
...
impl Handler<JarDataRequest> for JarManager {
    ...
    fn handle(&mut self, msg: JarDataRequest, _ctx: &mut Self::Context) -> Self::Result {
        ...
        let cache_file = match self.fs_cache.get_file(&jar_path) {
            ...
            Err(e) => {
                return AtomicResponse::new(Box::pin(std::future::ready(Err(Error::Other(
        ...
        fs.bytes(msg.path).map_err(|e| match e.kind() {
            ...
            _ => Error::Other(anyhow::Error::new(e).context("failed to get from memory cache")),
        })
    }
}
